...
while(a>0)
 ...
 for i = 1:size(x,1)
  if (doesBelongToS(x(i))==1)
     ...
     %       if astar == true
      %************* for A* ***************
    % distance between goal node and neighbour node
    % neighbour node = x(i)
...
if goalOrg(1)<boundary(4) && goalOrg(2)<boundary(5) && goalOrg(3) < boundary (6)&& goalOrg(1) ~= path(1,1) || goalOrg(1) ~= path(1,2) || goalOrg(1) ~= path(1,3)
